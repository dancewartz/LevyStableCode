...
%% Initializing parameters and setting up simulation
    ...
    for s = 1:length(ts)
        ...
        % off event
        uxc(ind_change) = uxc(ind_change) + status(ind_change).*(F*d./(4*pi*rs(ind_change).^2).*(3*(bx(ind_change).*xs(ind_change)./rs(ind_change) + by(ind_change).*ys(ind_change)./rs(ind_change) + bz(ind_change).*zs(ind_change)./rs(ind_change)).^2-1));
