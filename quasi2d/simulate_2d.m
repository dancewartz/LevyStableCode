...
rho = 0.5e0;    % number density of force dipoles (fillaments) per square micron;
L = 10;      % system size in microns
Npoints = round(rho*L^2);
m_monopole = 1;  % scaling for the monopole force response - should always be 1
d = 0.4;     % physical size of dipoles in micronsf
%saff_length = 1e3; % saffman delbruk length this is kappa^-1 in oppenheimer diamont paper
...
params.saff_length = saff_length; 
params.poisson_ratio = 0.5; % incompressible fluid

thin_by = 1;
burn = round(10/dt);  % burn-in time
[u,ts] = tensor_sum_2d(params);

save('simulation_data.mat','params','ts','u')
...
% clf
% histogram(u(burn/thin_by:thin_again:end),'normalization','pdf','linestyle','none','binedges',xf);
% hold on
% %xf = logspace(log10(xl(1)),log10(xl(2)),100);
